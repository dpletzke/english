...
import type { GameStatus, GuessFeedback, WordCard } from '../game/types'
...
export const useConnectionsGame = (puzzle: ConnectionsPuzzle): UseConnectionsGameResult => {
  ...
  const [mistakesRemaining, setMistakesRemaining] = useState<number>(mistakesAllowed)
  ...
  const submitSelection = () => {
    ...
    if (category && allSameCategory && targetCategoryId && !solvedSet.has(targetCategoryId)) {
    ...
    setMistakesRemaining((prev) => {
      ...
      setFeedback({ type: 'incorrect', remaining: next })
      return next
    })
    setSelectedWordIds([])
  }
